...
class _io:
    def __init__(self, cfg):
        ...
        name = amqp.get('name', DEFAULT_NAME)
        self.name = name.format(**D)
        D['name']=self.name

        self.exch = amqp.get('exchange', DEFAULT_EXCHANGE).format(**D)
        self.queue = amqp.get('queue', DEFAULT_QUEUE).format(**D)
        self.on_data = amqp.get('on', 'on')
        self.off_data = amqp.get('off', 'off')
        self.route = amqp.get('route', DEFAULT_ROUTE).format(**D)
+ name, dir=self.dir)



        self.json_path = amqp.get('json', None)
...
class Input(_io):
    ...
    async def run(self, amqp, chip, task_status=TASK_STATUS_IGNORED):
    ...
    async def handle_event(self, e):
        ...
        rkey = self.route.format(dir='in', chip=self.chip, pin=self.pin, name=self.name)
        data = data.format(dir='in', chip=self.chip, pin=self.pin, name=self.name)
        data = data.encode("utf-8")

        await chan.basic_publish(data, exchange_name=self.exch, routing_key = rkey)
...
class Output(_io):
    ...
    async def run(self, amqp, chip, task_status=TASK_STATUS_IGNORED):
        """Task handler for processing this output."""
        async with amqp.new_channel() as chan:
            await chan.exchange_declare(exchange_name=self.exch)
            await chan.queue_declare(queue_name=self.queue)
            rkey = self.route.format(dir='in', chip=self.chip, pin=self.pin)
            await chan.queue_bind(self.queue, self.exch, routing_key=rkey)
